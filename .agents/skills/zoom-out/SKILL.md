...
Adapted from [mattpocock/skills](https://github.com/mattpocock/skills/blob/main/skills/engineering/zoom-out/SKILL.md). Audited for this repo: replaced the upstream's `CONTEXT.md` reference with our actual context sources (`AGENTS.md` files + ADRs).

## Instructions

Go up a layer of abstraction. Give a map of the relevant modules and callers, using the language and conventions in:

1. The nearest `AGENTS.md` for the area being asked about (the per-app `apps/<product>/<service>/AGENTS.md` if one exists, otherwise the closest parent).
2. The root [AGENTS.md](../../AGENTS.md) for repo-wide conventions.
...
4. **Why it exists** — link the ADR or the AGENTS.md section that motivates it.
