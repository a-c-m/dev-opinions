...
`ApiException` + global exception filters (`AllExceptionsFilter` for REST, `GqlExceptionFilter` for GraphQL) per ADR 0019.
...
See [ADR 0019](../../docs/adr/0019-api-contracts-and-errors.md). 5xx `detail` is stripped to a generic message outside `APP_ENV=development`.
