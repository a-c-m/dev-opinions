...
// ADR 0017: in production the unplugin rewrites `import.meta.env.X` to a
...
// Build-time guard against the two ADR 0017 footguns that fail silently:
  ...
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      if (!String(html.source).includes("import_meta_env_placeholder")) {
        throw new Error(
          "[env-injection-guard] index.html is missing the runtime env placeholder. Add the <script> tag from ADR 0017 to <head> before the bundle script."
        );
      }
...
export default defineConfig({
  // ADR 0017: prevent Vite from auto-inlining custom env vars. Only Vite's
