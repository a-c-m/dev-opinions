...
# ADR 0040: E2E structure — per-app `-e2e` packages, fixtures-injected POM, project-scoped runs
...
- Stagehand stays opt-in under `<service>-e2e/stagehand/` per [ADR 0013](0013-vitest-playwright.md), gated by `STAGEHAND_MODE=1`.

## Context

[ADR 0013](0013-vitest-playwright.md) picked Playwright and asserted "E2E lives under `apps/<product>/<service>-e2e/`" without committing to the package shape, the page-object pattern, the project layout, or the retries/trace defaults. The sample app currently colocates `e2e/` inside the web app — the throwaway scaffold pattern, not the production one. Real services need a shared answer before the second `-e2e` package lands.
...
├── stagehand/                   # opt-in, gated by STAGEHAND_MODE env (ADR 0013)
└── tsconfig.json
...
export default defineConfig({
  ...
  use: {
    trace: "retain-on-failure-and-retries", // ADR 0040 — Playwright ≥1.59
...
Stagehand v3 lives at `apps/<p>/<s>-e2e/stagehand/`, ignored by the default `testDir`. Gated by `STAGEHAND_MODE=1` and a separate `playwright.stagehand.config.ts`. LLM cost and non-determinism keep it off the default gate per [ADR 0013](0013-vitest-playwright.md).

### What this defers (and the graduation trigger)

- **`staging-with-prod-auth` project** — implies a read-only prod token scoped per the synthetic-user model. Crosses [ADR 0037](0037-authentication.md) (token issuance) and [ADR 0025](0025-production-data-flow.md) (prod data egress). Graduation: when production runs and the token policy is approved.
...
4. **Stagehand as the default** (browserbase/stagehand v3). Rejected per [ADR 0013](0013-vitest-playwright.md): LLM latency + cost on the deterministic CI gate is a regression.
...
- [ADR 0013](0013-vitest-playwright.md) — picked Playwright; this ADR is the package + pattern detail.
- [ADR 0014](0014-package-by-feature.md) — `tests/<feature>/` mirrors the source package-by-feature shape.
- [ADR 0037](0037-authentication.md) — the `staging-with-prod-auth` graduation depends on the token policy here.
- [ADR 0025](0025-production-data-flow.md) — prod-auth E2E also needs sanctioned data-egress rules.
