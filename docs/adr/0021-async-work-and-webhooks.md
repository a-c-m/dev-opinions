...
# ADR 0021: Async work & external integrations — jobs and webhooks
...
All three share the same primitives: **durable state for in-flight work**, **idempotency keys**, **retry with backoff**, and **observability of work that doesn't appear in HTTP traces**. Sync-only resilience (rate limiting, caching) is in [ADR 0020](0020-request-cycle-resilience.md).
...
- **Observability**: each job execution is an OTel span ([ADR 0025](0025-runtime-observability.md)); queue depth + failure rate are Prometheus gauges.
- **Process model**: the worker is a separate NX project (`apps/<product>/worker/`) so it deploys and scales independently of HTTP services. The container shape is the standard one from [ADR 0034](0034-container-conventions.md).
...
1. **Validate signature** against the raw request body. The signature middleware must read the raw bytes — register it before any JSON parser. Provider-specific HMAC SHA-256 against a shared secret from [ADR 0026](0026-secrets-runtime-injection.md).
...
2. **BullMQ + Redis day-one** — Redis as new infra, queue semantics in a sidecar service. Rejected for the same reason Redis caching is rejected in [ADR 0020](0020-request-cycle-resilience.md) — solves a problem we don't have yet.
...
- [ADR 0009](0009-nestjs-backend.md) — Nest is the worker process framework; same DI patterns as HTTP services.
- [ADR 0011](0011-drizzle-orm.md) — Drizzle schemas for `jobs`, `webhook_inbox`, `webhook_outbox` live in `shared/db-<domain>/`.
- [ADR 0034](0034-container-conventions.md) — worker container shape (no HTTP, no `/healthz` requirement; readiness via Postgres ping).
- [ADR 0025](0025-runtime-observability.md) — jobs and webhook deliveries emit OTel spans + metrics.
- [ADR 0019](0019-api-contracts-and-errors.md) — webhook-in endpoints use the standard problem-details envelope for signature failures.
- [ADR 0026](0026-secrets-runtime-injection.md) — webhook signing keys are secrets, injected at runtime.
- [ADR 0018](0018-safe-database-migrations.md) — migrations for the three new tables follow the expand/contract rules.
- [ADR 0020](0020-request-cycle-resilience.md) — sync sibling.
