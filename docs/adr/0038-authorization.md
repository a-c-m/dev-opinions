...
[ADR 0037](0037-authentication.md) settles authn (who you are); this ADR settles authz (what you can do).

## Context

ADR 0037 puts `roles: string[]` on `TokenClaims` and explicitly defers authz. Every fork hits the gap on day two — "user can read their own orders, admins can read all" needs more than a role check. Without a convention, every service grows `if (user.roles.includes('admin'))` branches and IDOR (OWASP A01) bugs follow.

This ADR settles: the framework choice, the two-layer enforcement pattern, the file layout, frontend gating, and the audit-log seam. Stays scoped to authz; service-to-service auth and SCIM remain deferred per ADR 0037.

## Decision Outcome

### `@casl/ability` direct, factory hand-rolled

`@casl/ability` v6.8 (~950k weekly downloads) is the framework. The NestJS-side wiring is **hand-rolled** in `shared/authz/` rather than adopting `nest-casl` — fewer supply-chain surfaces and full control over hardening (deny-by-default, audit emission seam, error mapping into [ADR 0033](0033-api-contracts-and-errors.md)'s `ApiException`).
...
**Service (per-object, fine).** After the service loads the object, call `ability.can(action, loadedOrder)`. CASL checks attribute conditions (`{ userId: user.id }`) against the loaded record. Throw `ForbiddenException` if false; the [ADR 0033](0033-api-contracts-and-errors.md) `ApiException` filter maps this to `code: 'forbidden'`.
...
Services stay framework-free per ADR 0033 — the `AbilityFactory` is injected, but the `ability.can()` call is plain TS.
...
- **Error shape**: `ForbiddenException` maps via `GqlExceptionFilter` (ADR 0033) to a GraphQL error with `extensions.code = 'forbidden'`, `extensions.http.status = 403`, `extensions.traceId`. Same `code` discriminator the REST surface uses.
...
| **Entitlements / plan on token** | Plan changes require token re-issuance | Use [ADR 0037](0037-authentication.md)'s `EntitlementResolver`, not roles |
...
1. **`nest-casl` wrapper** — saves ~20 LOC of boilerplate; adds a supply-chain surface and an interceptor we don't need. Hand-rolled keeps full control over hardening and ADR 0033 error mapping.
...
- **[ADR 0010](0010-nestjs-backend.md)** — NestJS DI hosts `AbilityFactory` and `AuthzGuard`
- **[ADR 0011](0011-frontend-frameworks.md)** — React client uses `@casl/react`
- **[ADR 0012](0012-drizzle-orm.md)** — `audit_log` table (when implemented) follows the same Drizzle pattern
- **[ADR 0015](0015-backend-config.md)** — no `auth.*` config needed beyond ADR 0037
- **[ADR 0033](0033-api-contracts-and-errors.md)** — `ForbiddenException` maps to `code: 'forbidden'`; cross-surface error shape
- **[ADR 0037](0037-authentication.md)** — `TokenClaims.roles` feeds the factory; entitlements stay off-token
