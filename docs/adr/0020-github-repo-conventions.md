...
This ADR covers the repository-meta layer. CI workflows themselves are in [ADR 0021](0021-github-actions-ci.md).
...
- `@commitlint/cli` + `@commitlint/config-conventional` validate format via the Lefthook `commit-msg` hook ([ADR 0018](0018-lefthook.md)).
...
Deliberately **not** added. Per [ADR 0028](0028-multi-agent-rule-distribution.md), `AGENTS.md` is the canonical cross-agent context file; `CLAUDE.md` is a committed symlink to it. Mirroring into `copilot-instructions.md` guarantees drift. If Copilot users join the project, point them at `AGENTS.md`.
...
- [ADR 0018](0018-lefthook.md) — Lefthook hosts the `commit-msg` gate.
- [ADR 0021](0021-github-actions-ci.md) — CI workflows that consume the conventions here.
- [ADR 0024](0024-branching-releases-environments.md) — release flow that depends on Conventional Commits + CODEOWNERS review.
- [ADR 0028](0028-multi-agent-rule-distribution.md) — `AGENTS.md` cross-agent standard; explains why we don't mirror into per-agent files.
- [ADR 0030](0030-child-apps-and-repos.md) — scope: CODEOWNERS conventions apply to this repo; children under `repos/` own their own files.
