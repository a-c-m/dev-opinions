...
# ADR 0034: Container build and local dev conventions

## Context and Problem Statement

[ADR 0008](0008-trivy-security-scan.md) (Trivy scans),
[ADR 0032](0032-github-actions-ci.md) (CI builds),
[ADR 0033](0033-opentofu-iac.md) (IaC deploys),
[ADR 0004](0004-nx-monorepo.md) (`lint:container` verb — see [docs/conventions/scripts.md](../conventions/scripts.md)),
and [ADR 0036](0036-production-data-flow.md) (in-VPC sanitisation
...
# Vault CLI shim per ADR 0026. Pick the fork's vendor at build time.
...
# Stacking order: vault shim → OTel boot → app. See ADR 0026 + 0032.
...
injection via ECS `secrets:` block — see [ADR 0026](0026-secrets-runtime-injection.md))
...
- **Slim Debian** for every stage (glibc — `better-sqlite3` and
  other native modules from [ADR 0011](0011-drizzle-orm.md) work
...
- **`ENTRYPOINT` stacks vault → OTel → app.** `infisical run --`
  ...
  secrets ([ADR 0026](0026-secrets-runtime-injection.md)); `node --import` boots OTel before
  `@nestjs/core` is required ([ADR 0025](0025-runtime-observability.md));
...
([ADR 0035](0035-branching-releases-environments.md)).
...
parameterised in [ADR 0032](0032-github-actions-ci.md)); image
...
- **Frontend services** ([ADR 0010](0010-frontend-frameworks.md))
...
- **[ADR 0011](0011-drizzle-orm.md)** — native modules motivate slim/glibc over Alpine.
- **[ADR 0032](0032-github-actions-ci.md)** — CI builds this image.
- **[ADR 0035](0035-branching-releases-environments.md)** — release flow deploys it.
- **[ADR 0025](0025-runtime-observability.md)** —
