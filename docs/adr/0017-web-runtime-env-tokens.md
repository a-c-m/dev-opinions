...
# ADR 0017: Web runtime env injection via `import-meta-env`
...
(Backend services follow a different approach — see [ADR 0016](0016-backend-config.md). A briefly-shared earlier pattern covered both surfaces under one `src/env.ts` zod schema; it was retired in favour of these two separate decisions.)
...
- **Frontend-only** — Does not address backend/Node service config (covered by [ADR 0016](0016-backend-config.md)).
...
- [ADR 0016: Typed file-based config with secrets-only env vars (backend)](0016-backend-config.md) — server-side counterpart.
