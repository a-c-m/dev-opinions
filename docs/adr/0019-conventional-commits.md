...
# ADR 0019: Conventional Commits + commitlint + commitizen
...
- **Trailing issue suffix is mandatory for AI-authored commits**: `#<n>` (GitHub) or `PROJ-<n>` (Jira/Linear), e.g. `feat(api): add search endpoint #142`. The PreToolUse hook blocks `git commit` without one — there is no AI-side escape hatch. Humans MAY commit without one when there is genuinely no ticket (scaffolding, dep bump, hotfix-without-issue). See CLAUDE.md "Every commit names its ticket".
