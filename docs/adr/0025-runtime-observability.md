...
# ADR 0025: Runtime observability — OTel, metrics, tracing

## Context and Problem Statement

[ADR 0024](0024-structured-logging-contract.md) settles
...
|---|---|---|
| `pnpm dev` | App only; logs → `.ai-wip/logs/` per [ADR 0024](0024-structured-logging-contract.md). OTLP exporter targets `localhost:4318` and harmlessly fails if nothing is listening. | Default. Most local work. |
...
- **[ADR 0009](0009-nestjs-backend.md)** — NestJS hooks for OTel.
- **[ADR 0010](0010-frontend-frameworks.md)** — frontend SOPs over OTel web.
- **[ADR 0016](0016-backend-config.md)** — OTel vars follow the config/secret split.
- **[ADR 0033](0033-opentofu-iac.md)** — prod env vars set in Tofu.
- **[ADR 0034](0034-container-conventions.md)** — runtime
  `ENTRYPOINT` adds `--import ./dist/instrumentation.mjs`.
- **[ADR 0024](0024-structured-logging-contract.md)** — log shape and capture.
