...
# ADR 0020: Request-cycle resilience — rate limiting & caching
...
Async concerns (background jobs, webhooks, the outbox/inbox pair) are out of scope here — see [ADR 0021](0021-async-work-and-webhooks.md).
...
- **Identifier**: user ID when authenticated (via the `AuthOutcome` from [ADR 0027](0027-authentication.md)); IP fallback otherwise. Implemented via a small `ThrottlerGuard` subclass — `<10 LOC`.
- **Skip the health endpoints**: `@SkipThrottle()` on `/healthz` and `/readyz` per [ADR 0034](0034-container-conventions.md). Otherwise the orchestrator's probe traffic eats the budget.
...
- **Error shape**: 429 responses go through the global `ApiException` filter and emit RFC 9457 Problem Details per [ADR 0019](0019-api-contracts-and-errors.md), with `Retry-After` populated from the throttler.
- **Observability**: throttle decisions surface as OTel span attributes (`throttle.allowed`, `throttle.remaining`) per [ADR 0025](0025-runtime-observability.md). A counter metric tracks 429s.
...
Per [ADR 0010](0010-frontend-frameworks.md). Repo-wide defaults set once at the `QueryClient`:
...
A thin `@shared/cache` wrapper exposes `get(key)`, `set(key, value, ttl)`, `invalidate(key)`, `invalidatePrefix(prefix)`. Cleanup runs as a scheduled job ([ADR 0021](0021-async-work-and-webhooks.md)) — `DELETE WHERE expires_at < now()` nightly.
...
- **Error shape is unified.** 429s flow through the same problem-details envelope as every other error ([ADR 0019](0019-api-contracts-and-errors.md)).
...
- [ADR 0009](0009-nestjs-backend.md) — NestJS as the backend; throttler module lives here.
- [ADR 0010](0010-frontend-frameworks.md) — React + Vite; React Query is the client cache.
- [ADR 0034](0034-container-conventions.md) — `/healthz` and `/readyz` must be exempt from rate limits.
- [ADR 0019](0019-api-contracts-and-errors.md) — 429 problem-details shape.
- [ADR 0025](0025-runtime-observability.md) — throttle and cache OTel attributes.
- [ADR 0027](0027-authentication.md) — `AuthOutcome` drives the user-vs-IP throttle key.
- [ADR 0021](0021-async-work-and-webhooks.md) — async sibling; cache cleanup runs as a scheduled job.
