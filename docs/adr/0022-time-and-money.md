...
# ADR 0022: Time and money — UTC, minor units, never floats
...
- Zod ingress: `z.string().datetime({ offset: false })` validates inbound (per [ADR 0019](0019-api-contracts-and-errors.md)). Reject anything without the `Z` or with a local offset — the boundary is UTC.
...
- [ADR 0011](0011-drizzle-orm.md) — Drizzle column types (`timestamp({ withTimezone: true })`, `bigint({ mode: 'bigint' })`).
- [ADR 0019](0019-api-contracts-and-errors.md) — Zod schemas at REST boundaries; `MoneySchema` and `isoDateTime` are imported from `@shared/*`.
- [ADR 0026](0026-secrets-runtime-injection.md) — currencies have no secrets; locale + timezone are not configuration.
