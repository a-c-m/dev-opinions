...
- `FlagKey` union rejects unknown keys at typecheck on both
  ...
  in [ADR 0019](0019-api-contracts-and-errors.md).
...
the reverted long-lived-key hook in [ADR 0026](0026-secrets-runtime-injection.md):
...
[ADR 0025](0025-runtime-observability.md). Defer:
...
When a fork adopts PostHog per ADR 0025:
...
- **References [0009](0009-nestjs-backend.md),
  [0010](0010-frontend-frameworks.md),
  [0016](0016-backend-config.md),
  [0026](0026-secrets-runtime-injection.md)** — NestJS DI,
  React 19 hosting, provider keys via typed secrets +
  entrypoint shim.
- **Interacts with [0025](0025-runtime-observability.md)** —
  PostHog adoption triggers the asymmetric path.
- **Mirrors [0019](0019-api-contracts-and-errors.md)** — same
