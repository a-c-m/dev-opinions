...
# ADR 0023: File storage — S3-API as the contract, MinIO local, Postgres as truth
...
**CORS** on the upload bucket: allow the deployed origin in stage/prod (`https://app.example.com`); allow `http://localhost:5173` in local. Configured in IaC ([ADR 0033](0033-opentofu-iac.md)), not at runtime.
...
Trust client `Content-Type` only as a hint, even when enforced by the POST policy. On upload-complete, the client calls `POST /v1/files/{id}/finalize` which enqueues a Postgres-backed job ([ADR 0021](0021-async-work-and-webhooks.md)) that reads the first 4 KB via Range, runs the `file-type` package, and updates `files.detected_mime`. If the detected MIME contradicts the allowlist, mark the row `quarantined=true` and refuse all future presigned GETs.
...
Use `@fastify/multipart` for the rare server-side multipart receive path (Nest's `FileInterceptor` is multer-based and documented incompatible with `FastifyAdapter`). Storage injected via `@InjectStorage()` resolving to the `STORAGE_DRIVER` branded token. `/readyz` calls `driver.healthcheck()` and includes its result; `/healthz` stays liveness-only ([ADR 0034](0034-container-conventions.md)).
...
- [ADR 0016](0016-backend-config.md) — `@shared/config` selects the driver per `APP_ENV`; bucket names, region, endpoint, TTL defaults all live in the schema.
- [ADR 0033](0033-opentofu-iac.md) — bucket creation, CORS, lifecycle rules, KMS keys, IAM roles all in IaC.
- [ADR 0034](0034-container-conventions.md) — `/readyz` calls `driver.healthcheck()`.
- [ADR 0025](0025-runtime-observability.md) — OTel spans + metrics from `@shared/storage`.
- [ADR 0019](0019-api-contracts-and-errors.md) — RFC 9457 problem details for `quarantined`, presign-refused, etc.
- [ADR 0026](0026-secrets-runtime-injection.md) — AWS / R2 / B2 credentials via the vault CLI in `ENTRYPOINT`; never `.env`.
- [ADR 0027](0027-authentication.md) / [ADR 0028](0028-authorization.md) — `ability.can('read', file)` gates every presigned-GET issuance.
- [ADR 0021](0021-async-work-and-webhooks.md) — content-type detection, virus scanning, purge jobs all ride the Postgres jobs queue.
- [ADR 0022](0022-time-and-money.md) — `timestamptz` and `bigint` types on `files`.
