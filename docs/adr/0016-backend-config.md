...
# ADR 0016: Typed file-based config with secrets-only env vars (backend)

Replaces the previous per-app `src/env.ts` zod schema pattern that backend services briefly shared with web apps. The shared pattern is retired for backends; web apps now use [ADR 0017](0017-web-runtime-env-tokens.md), which owns its in-browser zod validator directly.
...
4. `pnpm dev` — the `dev` script wraps `op run --env-file=.env.example -- …` so secrets are injected at process start; no `.env` file is ever written to disk. See [ADR 0026](0026-secrets-runtime-injection.md).
...
Secrets reach containers via the entrypoint-shim CLI per [ADR 0026](0026-secrets-runtime-injection.md): a vault agent (`infisical run --` / `op run --` / etc.) runs inside the container at process start, authenticates as the workload, exports env vars, and `exec`s the app. `@shared/config` then reads `process.env` as normal.
...
- [ADR 0009](0009-nestjs-backend.md) — NestJS as the default backend framework
- [ADR 0017](0017-web-runtime-env-tokens.md) — frontend counterpart (browser env injection + zod validation)
