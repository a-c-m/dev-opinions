...
# ADR 0019: API contracts & error shapes
...
Two surfaces talk to clients: GraphQL (primary, per [ADR 0010](0010-frontend-frameworks.md)) and REST (narrow). Without shared contract / error / deprecation shapes, every service reinvents them and React can't discriminate errors uniformly across surfaces.
...
- **REST + boundaries** (config, headers, webhooks, queue messages, the `ApiError` response parsed by React): **Zod 4**. No `@InputType` class is mandatory; Zod's refinements / issue trees / `.nullable().optional()` win cleanly. Aligns with [ADR 0011](0011-drizzle-orm.md)'s `drizzle-zod`.
...
- `traceId` — W3C trace id from the active OTel span ([ADR 0025](0025-runtime-observability.md))
...
1. **Vocabulary as `readonly` tuples in Drizzle** (per [ADR 0011](0011-drizzle-orm.md)). Reused in `@IsIn([...TUPLE])` and service domain logic; never re-typed inline.
...
- **Trace correlation by construction** — `traceId` ties errors to ADR 0024 / 0032
...
- [ADR 0009](0009-nestjs-backend.md) — Yoga vs Apollo settled here (Yoga)
- [ADR 0010](0010-frontend-frameworks.md) — React consumes via codegen + TanStack Query
- [ADR 0011](0011-drizzle-orm.md) — Drizzle is the persistence + vocabulary truth
- [ADR 0024](0024-structured-logging-contract.md) + [ADR 0025](0025-runtime-observability.md) — `traceId` source
