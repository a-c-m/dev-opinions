...
| `*.resolver.ts`, `*.controller.ts` | smoke only — covered by E2E ([ADR 0013](0013-vitest-playwright.md)) | — | excluded from % rollup |
...
- Drizzle schema (`**/db/schema/**`) — declarative table defs ([ADR 0012](0012-drizzle-orm.md)).
- Configs, migrations, E2E specs, OTel boot (`instrumentation.mts` per [ADR 0032](0032-runtime-observability.md)).
...
- **Resolvers/controllers stop demanding unit-test theatre** — covered by E2E per [ADR 0013](0013-vitest-playwright.md).
...
- [ADR 0013](0013-vitest-playwright.md) — picked Vitest + v8; punted thresholds here.
- [ADR 0012](0012-drizzle-orm.md) — schema files are declarative; excluded from rollup.
- [ADR 0032](0032-runtime-observability.md) — `instrumentation.mts` is boot glue; excluded.
